...
module ReVIEW
  module Test
    # IdgxmlComparator compares IDGXML output from IDGXMLBuilder and IdgxmlRenderer
    # for testing purposes.
    class IdgxmlComparator
      ...
      def convert_chapter_with_book_context(book_dir, chapter_name)
        ...
        # Load book and find chapter for renderer
        book_for_renderer = load_book(book_dir)
        chapter_for_renderer = book_for_renderer.chapters.find { |ch| ch.name == chapter_name }
        raise "Chapter '#{chapter_name}' not found in book at #{book_dir}" unless chapter_for_renderer

        # Load book and find chapter for builder (separate instance)
        ...
        builder_idgxml = convert_with_builder(nil, chapter: chapter_for_builder)
        renderer_idgxml = convert_with_renderer(nil, chapter: chapter_for_renderer)
