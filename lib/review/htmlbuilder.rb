...
EOT
    ...
    def headline(level, label, caption)
      ...
      if caption.empty?
        ...
        if label.nil?
          puts %Q[<h#{level}>#{a_id}#{prefix}#{caption}</h#{level}>]
        else
          puts %Q[<h#{level} id="#{label}">#{a_id}#{prefix}#{caption}</h#{level}>]
        end
      end
    end
    ...
    def nonum_begin(level, label, caption)
      ...
      unless caption.empty?
        if label.nil?
          puts %Q[<h#{level}>#{caption}</h#{level}>]
        else
          puts %Q[<h#{level} id="#{label}">#{caption}</h#{level}>]
        end
      end
    end
    ...
    def captionblock(type, lines, caption)
      ...
      unless caption.nil?
        puts %Q[<p class="caption">#{caption}</p>]
      end
      lines.each {|l|
        puts "<p>#{l}</p>"
      }
    ...
    def list_header(id, caption)
      puts %Q[<p class="caption">リスト#{getChap}#{@chapter.list(id).number}: #{caption}</p>]
    end
...
QUOTE
    ...
    def image_header(id, caption)
      ...
      puts %Q[図#{getChap}#{@chapter.image(id).number}: #{caption}]
      puts %Q[</p>]
    end
    ...
    def table_header(id, caption)
      puts %Q[<p class="caption">表#{getChap}#{@chapter.table(id).number}: #{caption}</p>]
    end
    ...
    def footnote(id, str)
      puts %Q(<div class="footnote"><p class="footnote"><a id="fn-#{id}">[*#{@chapter.footnote(id).number}] #{str}</a></p></div>)
    end
    ...
    def numberlessimage(id, caption)
      puts %Q[<div class="image">]
      puts %Q[<img src="#{@chapter.image(id).path.sub(/^\.\//, "")}" alt="#{caption}" />]
      image_header id, caption
      puts %Q[</div>]
    end
