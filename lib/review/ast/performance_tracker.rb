...
module ReVIEW
  module AST
    # PerformanceTracker - Performance measurement and logging for AST compilation
    #
    # This class handles:
    ...
    class PerformanceTracker
      ...
      def log_statistics
        ...
        @logger.info('DEBUG: === Performance Statistics ===')
        @stats.each do |metric, value|
          if metric.to_s.include?('time')
            @logger.info("DEBUG:   #{metric}: #{(value * 1000).round(2)}ms")
          else
            @logger.info("DEBUG:   #{metric}: #{value}")
          end
        end
        @logger.info('DEBUG: ================================')
      end
